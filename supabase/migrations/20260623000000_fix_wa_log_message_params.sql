-- ============================================================
-- Fix: wa_log_message aceita p_push_name e p_remote_jid
-- BUG: n8n "Log Inbound Only" envia esses parametros extras
-- mas a funcao nao os aceitava, causando PGRST202 error.
-- Resultado: TODAS as mensagens inbound foram perdidas desde 05/04
-- ============================================================
...
BEGIN
  ...
  IF v_conv IS NULL THEN
    INSERT INTO wa_conversations (
      ...
      'active', p_persona, true, v_detected_name
    )
  ...
  ELSE
    ...
    IF v_conv.lead_id = 'unknown' AND v_lead_id IS NOT NULL THEN
      ...
      WHERE id = v_conv.id;
    END IF;
    -- Atualizar display_name com push_name se estava vazio
    IF v_detected_name IS NOT NULL AND (v_conv.display_name IS NULL OR v_conv.display_name = '' OR v_conv.display_name = 'Desconhecido') THEN
      UPDATE wa_conversations
      SET display_name = v_detected_name, updated_at = v_now
      WHERE id = v_conv.id;
    END IF;
  END IF;
  ...
  -- Atualizar lead (incluindo nome do push_name)
...
GRANT EXECUTE ON FUNCTION wa_log_message(text, text, text, text, int, text, text, text, text) TO anon, authenticated;
