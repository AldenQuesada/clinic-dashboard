-- ============================================================================
-- Beauty & Health Magazine — Templates t21 + t22 (Product Spread)
-- ============================================================================
-- t21_product_photo_split: 2 fotos lado a lado, fundo creme, nome do produto
-- t22_product_feature_text: matéria full-text 2 colunas, par com t21
-- Atualiza tambem magazine_validate_section com cases para os 2 novos slugs.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 1) Seed dos 2 templates novos
-- ----------------------------------------------------------------------------
INSERT INTO public.magazine_templates (slug, name, category, slots_schema, html_template)
VALUES
  ('t21_product_photo_split', 'Produto · 2 Fotos Split', 'feature',
    '{"required":["foto_principal","foto_detalhe","kicker","nome_produto"],"optional":["legenda_principal","legenda_detalhe","tagline"]}'::jsonb,
    '<!-- render server-side: ver MagazineRenderer.t21_product_photo_split -->'),

  ('t22_product_feature_text', 'Produto · Matéria Texto', 'feature',
    '{"required":["kicker","titulo","lede","corpo"],"optional":["byline","destaque"]}'::jsonb,
    '<!-- render server-side: ver MagazineRenderer.t22_product_feature_text -->')
...
-- ----------------------------------------------------------------------------
-- 2) Atualiza validator com cases para t21 e t22
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  -- ===================== NOVOS: t21 + t22 (spread de produto) =====================

  WHEN 't21_product_photo_split' THEN
    v_required := ARRAY['foto_principal','foto_detalhe','kicker','nome_produto'];
    IF length(COALESCE(p_slots->>'kicker','')) > 22 THEN v_errors := array_append(v_errors, 'kicker > 22 chars'); END IF;
    IF length(COALESCE(p_slots->>'nome_produto','')) > 40 THEN v_errors := array_append(v_errors, 'nome_produto > 40 chars'); END IF;
    IF length(COALESCE(p_slots->>'tagline','')) > 60 THEN v_errors := array_append(v_errors, 'tagline > 60 chars'); END IF;
    IF length(COALESCE(p_slots->>'legenda_principal','')) > 80 THEN v_errors := array_append(v_errors, 'legenda_principal > 80 chars'); END IF;
    IF length(COALESCE(p_slots->>'legenda_detalhe','')) > 80 THEN v_errors := array_append(v_errors, 'legenda_detalhe > 80 chars'); END IF;
  ...
  WHEN 't22_product_feature_text' THEN
    ...
    IF length(COALESCE(p_slots->>'lede','')) > 200 THEN v_errors := array_append(v_errors, 'lede > 200 chars'); END IF;
    IF length(COALESCE(p_slots->>'lede','')) < 100 THEN v_warnings := array_append(v_warnings, 'lede curto (< 100 chars)'); END IF;
    IF length(COALESCE(p_slots->>'destaque','')) > 140 THEN v_errors := array_append(v_errors, 'destaque > 140 chars'); END IF;
    v_val := p_slots->>'corpo';
    IF v_val IS NOT NULL THEN
      v_len := array_length(regexp_split_to_array(v_val, '\s+'), 1);
      IF v_len < 400 THEN v_warnings := array_append(v_warnings, 'corpo curto (< 400 palavras)'); END IF;
  ...
  -- Check emoji em texto editorial
