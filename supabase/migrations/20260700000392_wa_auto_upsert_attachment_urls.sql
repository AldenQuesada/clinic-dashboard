-- ============================================================
-- Migration: wa_agenda_auto_upsert aceita attachment_urls (galeria)
-- ============================================================
-- Extende a RPC pra propagar o novo campo attachment_urls jsonb.
-- Retrocompat: se payload nao traz o campo, preserva o valor atual.
-- ============================================================

-- Atualiza list pra incluir attachment_urls no retorno
...
BEGIN
  RETURN ARRAY(
    SELECT jsonb_build_object(
      ...
      'attachment_urls',       COALESCE(a.attachment_urls, '[]'::jsonb),
...
BEGIN
  ...
  -- attachment_urls: aceita null, array ou array vazio. null = preserva valor atual.
  v_atts := CASE
    WHEN p_data ? 'attachment_urls' AND jsonb_typeof(p_data->'attachment_urls') = 'array'
      THEN p_data->'attachment_urls'
    ELSE NULL
  END;
  ...
  IF v_id IS NOT NULL THEN
    UPDATE public.wa_agenda_automations SET
      ...
      attachment_urls        = COALESCE(v_atts, attachment_urls),
  ...
  ELSE
    ...
    INSERT INTO public.wa_agenda_automations (
      ...
      attachment_url, attachment_urls, attachment_above_text
    ) VALUES (
