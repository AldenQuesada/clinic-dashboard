-- Fix: kanban evolution missing idade
...
-- Fix: quiz_responses missing idade column
ALTER TABLE quiz_responses ADD COLUMN IF NOT EXISTS idade int;

-- Fix: submit saves idade to quiz_responses too
...
DECLARE
  ...
  v_recent_count int;
BEGIN
  -- Rate limit: max 5 submissions per phone per hour
  IF p_contact_phone IS NOT NULL AND trim(p_contact_phone) <> '' THEN
    ...
    IF v_recent_count >= 5 THEN
      RETURN jsonb_build_object('error', 'rate_limit', 'message', 'Muitas tentativas. Tente novamente em alguns minutos.');
    END IF;
  END IF;
