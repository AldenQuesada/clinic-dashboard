/**
 * B2BMTimeseriesWidget — histórico de vouchers emitidos × resgatados.
 * Consome B2BMetricsRepository.timeseries(bucket, periods, pid) → RPC b2b_timeseries.
 * Renderiza SVG inline com 2 linhas suavizadas + eixo + legenda + toggle Dia/Semana/Mês.
 */
;(function () {
  'use strict'
  if (window.B2BMTimeseriesWidget) return
  ...
  function _header() {
    ...
    return '' +
      '<div class="b2bm-ts-head">' +
        '<div>' +
          '<div class="b2bm-widget-title">Histórico de vouchers</div>' +
          '<div class="b2bm-widget-sub">Últimos ' + _state.periods + ' ' + b.label.toLowerCase() + 's</div>' +
        '</div>' +
        '<div class="b2bm-ts-bucket-buttons">' +
          BUCKETS.map(function (opt) {
            return '<button type="button" class="b2bm-ts-bucket' +
  ...
  function _renderLoading(host) {
    host.innerHTML = _header() + '<div class="b2bm-widget-loading">Carregando…</div>'
  }

  function _renderError(host, msg) {
    host.innerHTML = _header() +
      '<div class="b2bm-widget-err">Falha ao carregar série: ' + _esc(msg || 'erro desconhecido') + '</div>'
  }
  ...
  function _renderData(host, series) {
    if (!Array.isArray(series) || series.length === 0) {
      host.innerHTML = _header() + '<div class="b2bm-empty">Sem dados suficientes.</div>'
      return
    }
    ...
    var yGrid = yTicks.map(function (v) {
      ...
      return '' +
        '<line class="b2bm-ts-axis" x1="' + PAD_L + '" x2="' + (VB_W - PAD_R) + '" y1="' + y + '" y2="' + y + '" />' +
        '<text class="b2bm-ts-label" x="' + (PAD_L - 6) + '" y="' + (Number(y) + 3) + '" text-anchor="end">' + _esc(v) + '</text>'
    }).join('')
    ...
    var xLabels = series.map(function (p, i) {
      ...
      return '<text class="b2bm-ts-label" x="' + x + '" y="' + (VB_H - 8) + '" text-anchor="middle">' +
    ...
    var svg = '' +
      '<svg class="b2bm-ts-chart" viewBox="0 0 ' + VB_W + ' ' + VB_H + '" preserveAspectRatio="none" role="img" aria-label="Série temporal de vouchers">' +
        yGrid +
        '<path class="b2bm-ts-area" d="' + areaCreated + '" />' +
        '<path class="b2bm-ts-line" d="' + lineCreated + '" stroke="' + COLOR_CREATED + '" />' +
        '<path class="b2bm-ts-line" d="' + lineClosed  + '" stroke="' + COLOR_CLOSED  + '" />' +
        dotsClosed +
        xLabels +
      '</svg>' +
      '<div class="b2bm-ts-legend">' +
        '<span><i style="background:' + COLOR_CREATED + '"></i>Vouchers emitidos</span>' +
        '<span><i style="background:' + COLOR_CLOSED  + '"></i>Vouchers resgatados</span>' +
      '</div>'
  ...
  async function _load(host) {
    ...
    try {
      ...
      var data = await window.B2BMetricsRepository.timeseries(_state.bucket, _state.periods, _state.partnerId || null)
  ...
  window.B2BMTimeseriesWidget = Object.freeze({ mount: mount })
})()
