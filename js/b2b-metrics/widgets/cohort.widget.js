/**
 * B2BMCohortWidget — retenção de parcerias B2B por coorte mensal.
 * Consome B2BMetricsRepository.cohort(6) → RPC b2b_cohort_retention.
 * Renderiza matriz coorte × M+0..M+6 com heatmap via CSS var --heat.
 */
;(function () {
  'use strict'
  if (window.B2BMCohortWidget) return
  ...
  function _header() {
    return '' +
      '<div class="b2bm-widget-title">Retenção de parcerias · 6 meses</div>' +
      '<div class="b2bm-widget-sub">Coortes mensais × sobrevivência</div>'
  }
  ...
  function _renderError(host, msg) {
    host.innerHTML = _header() +
      '<div class="b2bm-widget-err">Falha ao carregar coortes: ' + _esc(msg || 'erro desconhecido') + '</div>'
  }
  ...
  function _cellHtml(row, col) {
    ...
    return '<td class="b2bm-cohort-cell" style="--heat:' + heat.toFixed(0) + '">' + inner + '</td>'
  }
  ...
  async function mount(hostId) {
    ...
    try {
      ...
      var data = await window.B2BMetricsRepository.cohort(6)
  ...
  window.B2BMCohortWidget = Object.freeze({ mount: mount })
})()
