...
;(function () {
    ...
    if (finalScore !== null) {
      ...
      html += '<div style="text-align:center;padding:28px 32px 20px 32px">' +
        '<div style="display:inline-flex;align-items:center;justify-content:center;width:100px;height:100px;border-radius:50%;border:4px solid ' + hsColor + ';box-shadow:0 0 30px ' + hsColor + '30">' +
          ...
          '<div style="font-size:10px;color:rgba(245,240,232,0.4);letter-spacing:0.1em">/100</div></div>' +
        '</div>' +
        '<div style="font-family:Cormorant Garamond,serif;font-size:18px;font-style:italic;color:' + hsColor + ';margin-top:10px">' + hsLabel + '</div>' +
        '<div style="font-size:10px;color:rgba(245,240,232,0.25);margin-top:4px;letter-spacing:0.1em;text-transform:uppercase">Indice de Harmonia Facial</div>' +
      '</div>'
    }
      ...
      html += '<div style="display:flex;align-items:flex-start;gap:8px;padding:5px 8px;background:' + (i % 2 === 0 ? 'rgba(255,255,255,0.02)' : 'transparent') + ';border-radius:4px">' +
        ...
        '<div style="flex:1;min-width:0">' +
          '<div style="display:flex;justify-content:space-between;align-items:center">' +
            ...
            '</div>' +
          '</div>' +
          (phrase ? '<div style="font-size:10px;font-style:italic;color:rgba(200,169,126,0.45);margin-top:1px">' + phrase + '</div>' : '') +
        '</div>' +
      '</div>'
    })
    html += '</div>'
  ...
  function _buildHTMLComparator(angleKey, angleLabel) {
    var beforeSrc = FM._photoUrls && FM._photoUrls[angleKey]
    var afterSrc = FM._afterPhotoByAngle && FM._afterPhotoByAngle[angleKey]
  ...
  FM._exportReportHTML = function () {
    ...
    function finalize() {
      ...
      var fullHtml = '<!DOCTYPE html><html lang="pt-BR"><head>' +
        ...
        content +
        '</div>' +
        // ── Interactive ANTES/DEPOIS Comparators (all angles) ──
        (function () {
          var comparatorAngles = [
            { key: 'front', label: 'Frontal' },
            { key: '45', label: '45\u00B0' },
            { key: 'lateral', label: 'Lateral' }
          ]
          var html = ''
          comparatorAngles.forEach(function (a) {
            if (FM._photoUrls && FM._photoUrls[a.key] && FM._afterPhotoByAngle && FM._afterPhotoByAngle[a.key]) {
              html += _buildHTMLComparator(a.key, a.label)
            }
          })
          return html
        })() +
        ...
        reader.onload = function () {
          ...
          fetch(EVOLUTION_URL + '/message/sendMedia/' + EVOLUTION_INSTANCE, {
            ...
            body: JSON.stringify({
              ...
              mediatype: 'document',
              mimetype: 'text/html',
              caption: FM.WA_REPORT_CAPTION ? FM.WA_REPORT_CAPTION.replace('{nome}', patientName) : 'Plano de Harmonia Facial personalizado para ' + patientName + '\n\nClinica Mirian de Paula\nHarmonia que revela. Precisão que dura.',
              media: base64,
      ...
      imgReader.onload = function () {
        ...
        fetch(EVOLUTION_URL + '/message/sendMedia/' + EVOLUTION_INSTANCE, {
          ...
          body: JSON.stringify({
            ...
            mediatype: 'image',
            mimetype: 'image/jpeg',
            caption: FM.WA_IMAGE_CAPTION || 'Resultado do seu Protocolo de Harmonia Facial\n\nClinica Mirian de Paula\nHarmonia que revela. Precisão que dura.',
            media: imgBase64,
