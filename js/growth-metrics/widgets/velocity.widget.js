/**
 * VpiVelocityWidget — tempo médio até conversão + delta vs período anterior.
 * Consome GrowthMetricsRepository.velocity(days).
 * IIFE puro.
 */
;(function () {
  'use strict'
  if (window.VpiVelocityWidget) return
  ...
  function _header() {
    return '' +
      '<div class="gm-widget-title">Velocity · tempo até conversão</div>' +
      '<div class="gm-widget-sub">Média, range e tendência</div>'
  }

  function _renderLoading(host) {
    host.innerHTML = _header() + '<div class="gm-widget-loading">Calculando velocity…</div>'
  }
  ...
  function _renderData(host, data) {
    ...
    var html = _header() +
      '<div class="gm-kpi-grid">' +
        '<div class="gm-kpi">' +
          '<div class="gm-kpi-val">' + _fmt(avg, 1) + '<span style="font-size:14px;color:var(--ink-muted);margin-left:6px;">dias</span></div>' +
          '<div class="gm-kpi-lbl">Média até conversão</div>' +
          '<div class="gm-kpi-sub gm-kpi-delta ' + deltaCls + '">' + deltaArr + ' ' + deltaTxt + ' vs período anterior</div>' +
        '</div>' +
        '<div class="gm-kpi">' +
          '<div class="gm-kpi-val" style="font-size:22px;">' + _fmt(min, 1) + ' – ' + _fmt(max, 1) + '<span style="font-size:12px;color:var(--ink-muted);margin-left:6px;">dias</span></div>' +
          '<div class="gm-kpi-lbl">Range (mín – máx)</div>' +
          '<div class="gm-kpi-sub">' + n + ' indicações fechadas no período</div>' +
        '</div>' +
      '</div>'
    host.innerHTML = html
  }
  ...
  window.VpiVelocityWidget = Object.freeze({ mount: mount })
})()
