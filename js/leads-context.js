...
;(function () {
  ...
  function _createInstance(cfg) {
    ...
    function _updateBulkBar() {
      ...
      _$('BulkTemp').onclick = function() {
        ...
        _selectedIds.forEach(function(id) {
          ...
          if (idx >= 0) { leads[idx].temperature = temp; localStorage.setItem('clinicai_leads', JSON.stringify(leads)) }
        })
    ...
    function _bulkChangePhase(ids, newPhase) {
      ...
      // Atualizar localStorage imediatamente (UI responsiva)
      ...
      _load() // Refresh imediato da lista
      ...
      if (sb) {
        ...
        sbPromise = sb.rpc('leads_bulk_change_phase', { p_ids: ids, p_phase: newPhase }).then(function(res) {
          if (res.error) {
            ...
            return _bulkChangePhaseFallback(sb, ids, newPhase)
          }
        ...
        sbPromise = _bulkChangePhaseFallback(null, ids, newPhase)
      }

      // Aguardar Supabase update ANTES de re-sync (evita sobrescrever com dados velhos)
    ...
    function _bulkChangePhaseFallback(sb, ids, newPhase) {
      var promises = ids.map(function(id) {
        if (window.SdrService) {
          return SdrService.changePhase(id, newPhase, 'bulk_move')
        }
        if (sb) {
          return sb.rpc('sdr_change_phase', { p_lead_id: id, p_to_phase: newPhase, p_reason: 'bulk_move' })
        }
